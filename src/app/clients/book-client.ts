export const BookClient = {
  ...
  markUnRead: async (olid: string) => {
    // This allows user to mark a book as unread
    const response = await fetch("/api/user/books/read", {
      method: "PATCH",
  ...
  markUndoWantToRead: async (olid: string) => {
    // This allows user to undo a want to read action
    const response = await fetch("/api/user/books/want-to-read", {
      method: "PATCH",
